...
static void generate_coinbase(ckpool_t *ckp, workbase_t *wb)
{
	...
	memcpy(wb->coinb2bin, "\x66\x72\x61\x63\x74\x61\x6C", 8);
	wb->coinb2len = 8;
	if (ckp->btcsig) {
...
static void client_apply_mindiff_override(stratum_instance_t  *client)
{
    ...
    size_t useragent_len = strlen(client->useragent);

    // NB: 所有的覆盖规则在这里已经确保在矿池的全局最小难度和最大难度之间
    for (unsigned i = 0; i < ckp->n_mindiff_overrides; ++i) {
        // 获取当前的覆盖规则
        const mindiff_override_t * const ovr = ckp->mindiff_overrides + i;

        // 比较前缀前，先确保客户端的useragent长度足够
        if (useragent_len >= ovr->ualen && 0 == strncasecmp(client->useragent, ovr->useragent, ovr->ualen)) {
            // 匹配成功，应用最小难度到客户端
            client->suggest_diff = client->old_diff = client->diff = ovr->mindiff;
            LOGDEBUG("mindiff_overrides: Applied minimum & starting difficulty = %"PRId64" to client %"PRId64" matching \"%s\"", 
                     ovr->mindiff, client->id, ovr->useragent);
            return;
        }
    }

    // 如果没有找到匹配项，可以输出调试日志
    LOGDEBUG("mindiff_overrides: No matching minimum difficulty override for client %"PRId64" with useragent \"%s\"",
                              ...
                              const char *nonce, uchar *hash, const bool stale)
{
    ...
    // 计算coinbase所需的大小
   size_t coinbase_size = wb->coinb1len + wb->enonce1constlen + wb->enonce1varlen + 
                       wb->enonce2varlen + 142 + 26 + wb->coinb3len;
    
    // 调试输出：检查 coinbase_size 和其他关键值
    // printf("coinbase_size = %zu, wb->coinb1len = %d, enonce1len = %d, enonce2len = %d, coinb2len = %d\n", 
    //        coinbase_size, wb->coinb1len, wb->enonce1constlen + wb->enonce1varlen, 
    //        wb->enonce2varlen, wb->coinb2len);
    ...
    // 检查并复制 enonce1 数据
    // printf("Before memcpy (enonce1): cblen = %d, required = %d, remaining buffer = %zu\n", 
    //        cblen, wb->enonce1constlen + wb->enonce1varlen, coinbase_size - cblen);

    if (cblen + wb->enonce1constlen + wb->enonce1varlen > coinbase_size) {
        fprintf(stderr, "Error: memcpy would overflow the coinbase buffer (enonce1). cblen = %d, required size = %d\n", 
                cblen, wb->enonce1constlen + wb->enonce1varlen);
        free(coinbase);
        return -1;
    }
    ...
    // 检查并复制 nonce2 数据
    // printf("Before memcpy (nonce2): cblen = %d, required = %d, remaining buffer = %zu\n", 
    //        cblen, wb->enonce2varlen, coinbase_size - cblen);

    if (cblen + wb->enonce2varlen > coinbase_size) {
        fprintf(stderr, "Error: memcpy would overflow the coinbase buffer (nonce2). cblen = %d, required size = %d\n", 
                cblen, wb->enonce2varlen);
        free(coinbase);
        return -1;
    }
    ...
    // printf("Before memcpy (coinb2): cblen = %d, required = %d, remaining buffer = %zu\n", 
    //        cblen, cb2len, coinbase_size - cblen);

    if (cblen + cb2len > coinbase_size) {
        fprintf(stderr, "Error: memcpy would overflow the coinbase buffer (coinb2). cblen = %d, required size = %d\n", 
                cblen, cb2len);
        ck_runlock(&sdata->instance_lock);
        free(coinbase);
        return -1;
    ...
    // 复制缓存的头部二进制并插入 merkle 根
    memcpy(data, wb->headerbin, 80);
    memcpy(data + 36, merkle_root, 32);

    // 当使用 version_mask 时更新 nVersion
    if (version_mask) {
        version_mask = htobe32(version_mask);
        data32 = (uint32_t *)data;
        *data32 |= version_mask;
